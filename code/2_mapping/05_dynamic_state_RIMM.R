...
for (i in 1:length(timeseries)){
  ...
  if (!"pm25_state" %in% names(static)){
    message("Kriging initial state map...")
    static$pm25_state = ord_krig(
      x = df_now_sf |> filter(!is.na(pm25)), 
      var="pm25", template = static, 
      nmin = 15, nmax = 30)
  }
  ...
  df_now_sf = bind_cols(df_now_sf |> select(time, pm25_smoothed), 
                        cov_ext) |> 
    tidyr::drop_na()
  
  # linear model
  LM = lm(log(pm25_smoothed) ~ Elevation + Population + Highway + Major_Roads + 
            Minor_Roads + CLC_Nat + rh + t2m + wd + ws + cams_pm25 +
            pm25_lag1 + pm25_lag24 + pm25_roll_8h, data = df_now_sf)
  #summary(LM)
  ...
  write_stars(cov["pred"][fa[1,]], paste0("maps/map_", method, "_pm25_single_", as.integer(t), ".tif"))
  
  #plot_pm25_pred(cov)

  # state map
  state_map <- alpha * cov["pred"] + (1 - alpha) * cov["pm25_state"]
  write_stars(state_map[fa[1,]], paste0("maps/map_", method, "_pm25_state_", as.integer(t), ".tif"))
  static["pm25_state"] = state_map
  
  #plot_pm25_pred(cov)
  
  # validation metrics
  val = eea |> filter(time == as.integer(t))
  val$pred = st_extract(state_map, val) |> pull("pred")
  ...
  metrics = bind_rows(metrics, 
                      ...
                      list(time = as.character(t), cv = "model", 
                           model_measures(LM$model$`log(pm25_smoothed)`, 
