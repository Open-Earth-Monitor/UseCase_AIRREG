...
for (i in 1:length(timeseries)){
  ...
  df_now_sf = bind_cols(df_now_sf |> select(time, pm25), cov_ext) |> 
    tidyr::drop_na()
  
  # RF model
  fit_classic = randomForest(
  ...
  cov = cov[fa[1,]]
  cov$rf_pred = predict(fit_classic, 
    newdata = model.matrix(~ . - 1, data = as.data.frame(cov) |> 
                             select(-x,-y))) |> 
    as.vector() |> exp()
